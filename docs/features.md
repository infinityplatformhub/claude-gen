...
### Auto-Skill Activation (เลือกตอน init)

Skills เป็น **model-invoked** (Claude เรียกเองเมื่องานตรง description) ไม่ได้รันอัตโนมัติ
เปิด auto-skill ตอน init เพื่อเพิ่มความน่าเชื่อถือ 2 ชั้น:
- **Skill Routing table** ใน CLAUDE.md (always-loaded nudge: "งานแบบนี้ → เรียก skill นี้")
- **UserPromptSubmit hook** ใน `.claude/settings.json` ที่ฉีดรายชื่อ skill เข้า context ทุก prompt
